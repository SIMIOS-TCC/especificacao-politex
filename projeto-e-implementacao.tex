...
O projeto Simio-Tx representa o target. Ele é responsável por emitir pacotes com a identificação do macaco em que está instalado (pacote 1) e possui a função de Task\texttt{\char`_}Sleep() para diminuir o consumo energético. \\
...
Já o projeto AP\texttt{\char`_}central\texttt{\char`_}RxUART recebe as informações de monitoramento dos três pontos de acesso, e assim as envia por UART para o Raspberry Pi. \\
O código elaborado de cada projeto é acessível no link da organização do github do grupo \cite{github}.\\
